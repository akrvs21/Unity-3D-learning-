...
>>>>>>> origin/master
    ...
    public GameObject GetDisk(int round)
    {
        ...
        switch (round)
        {
            ...
            case 1:
                {
                    newDisk.GetComponent<DiskData>().color = Color.grey;
                    newDisk.GetComponent<DiskData>().speed = 6.5f;
                    float RanX = UnityEngine.Random.Range(-1f, 1f) < 0 ? -1 : 1;
                    newDisk.GetComponent<DiskData>().direction = new Vector3(RanX, 1, 0);
                    newDisk.GetComponent<Renderer>().material.color = Color.grey;
                    break;
                }
            case 2:
                {
                    newDisk.GetComponent<DiskData>().color = Color.green;
                    newDisk.GetComponent<DiskData>().speed = 8.5f;
                    float RanX = UnityEngine.Random.Range(-1f, 1f) < 0 ? -1 : 1;
                    newDisk.GetComponent<DiskData>().direction = new Vector3(RanX, 1, 0);
